...
func (h *Handler) handleGetFiles(c *gin.Context) {
	...
	if isAdmin {
		database.RODB.Get(&storageUsed, "SELECT COALESCE(SUM(size), 0) FROM files WHERE is_folder = 0 AND message_id IS NOT NULL AND deleted_at IS NULL")
	} else {
		prefix := "/" + username
		database.RODB.Get(&storageUsed, "SELECT COALESCE(SUM(size), 0) FROM files WHERE (path = ? OR path LIKE ?) AND is_folder = 0 AND message_id IS NOT NULL AND deleted_at IS NULL", prefix, prefix+"/%")
	}
