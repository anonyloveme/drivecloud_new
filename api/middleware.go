	...
	return func(c *gin.Context) {
		...
		if !tgclient.IsSystemReady() {
			...
			if tgclient.IsRunning() && !tgclient.IsInitializationDone() {
				fmt.Println("[Drive] Telegram client is initializing. Serving 'Drive is starting up' page to:", c.Request.URL.Path)
				c.Data(http.StatusOK, "text/html; charset=utf-8", []byte(`
						...
						</head><body><div><h2>Drive is starting up</h2><p>Please wait a few seconds...</p></div></body></html>
					`))
				c.Abort()
				return
			}
