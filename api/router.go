...
func SetupRouter(cfg *config.Config, contentFS fs.FS, startTG func(cfg *config.Config), restartApp func()) *gin.Engine {
	...
	// Heavy public endpoints — rate-limited (60 req/min per IP)
	rl := r.Group("", publicRateLimit())
	rl.GET("/s/:token/stream", h.handleStreamSharedFile)
	rl.GET("/s/:token/stream/:filename", h.handleStreamSharedFile)
	...
	rl.GET("/s/:token/file/:id/cbz/list", h.handleGetSharedComicPages)
	rl.GET("/s/:token/file/:id/cbz/page", h.handleGetSharedComicPage)
	rl.HEAD("/s/:token/file/:id/cbz/page", h.handleGetSharedComicPage)
	rl.GET("/s/:token/epub/meta", h.handleGetSharedEpubMeta)
	rl.GET("/s/:token/epub/resource/*path", h.handleGetSharedEpubResource)
	rl.HEAD("/s/:token/epub/resource/*path", h.handleGetSharedEpubResource)
	rl.GET("/s/:token/file/:id/epub/meta", h.handleGetSharedEpubMeta)
	rl.GET("/s/:token/file/:id/epub/resource/*path", h.handleGetSharedEpubResource)
	rl.HEAD("/s/:token/file/:id/epub/resource/*path", h.handleGetSharedEpubResource)
	rl.GET("/dl/:token", h.handleGetDirectDownload)
	...
	r.POST("/download/bulk", authMiddleware(), h.handleDownloadBulk)
