...
func Load() (*Config, error) {
	...
	uploadThreads, _ := strconv.Atoi(getEnv("TG_UPLOAD_THREADS", "4"))
	if uploadThreads <= 0 {
		uploadThreads = 4
	}

	uploadConcurrency, _ := strconv.Atoi(getEnv("TG_UPLOAD_CONCURRENCY", "6"))
	if uploadConcurrency < 0 {
		uploadConcurrency = 0 // 0 = auto (bots + 1)
	}

	uploadStaggerMs, _ := strconv.Atoi(getEnv("TG_UPLOAD_STAGGER_MS", "300"))
	...
	return &Config{
		...
		UploadStaggerMs:    uploadStaggerMs,
